class Arrear < ActiveRecord::Base
  ...
  validates :employee_id, :company_id, :amount, :description, :arrear_at, :presence => true
  ...
  scope :arrear_in, lambda{|e|{:conditions => ["month(arrear_at) = ? and year(arrear_at) = ?",e.month,e.year]}}
  scope :unbilled, :conditions => ["salary_slip_id is null"]
  scope :in_years, lambda{|yrs|{:conditions => ["year(arrear_at) in (?)",yrs]}}
  scope :in_months,lambda{|mnth|{:conditions => ["month(arrear_at) in (?)", mnth]}}

  def self.finalize_for_slip!(run_date,salary_slip,employee)
    for_employee(employee).arrear_in(run_date).unbilled.each {|d| d.update_attribute(:salary_slip_id,salary_slip.id)}
  end
  ...
  def self.employee_month_wise(employee)
    for_employee(employee).sum(:amount,:group => "date_format(arrear_at,'%b %Y')")
  end
  ...
  def self.search(company,year=Date.today.year,employee=nil)
    ...
    result = result.select{|x|x.arrear_at.financial_year == year}
    result.group_by{|ex| ex.arrear_at.month}
  end
