class ExpenseClaim < ActiveRecord::Base
  ...
  validates :employee_id, :company_id, :amount, :description, :expensed_at, :presence => true
  ...
  scope :expensed_in, lambda{|e|{:conditions => ["month(expensed_at) = ? and year(expensed_at) = ?",e.month,e.year]}}
  scope :unbilled, :conditions => ["salary_slip_id is null"]
  scope :in_years, lambda{|yrs|{:conditions => ["year(expensed_at) in (?)",yrs]}}
  scope :in_months,lambda{|mnth|{:conditions => ["month(expensed_at) in (?)", mnth]}}
