class SalaryHead < ActiveRecord::Base
  ...
  attr_accessible :name, :code, :tax_formula

  has_many :salary_slip_charges

  validates :name, :code, :tax_formula, :presence => true

  scope :allowance_compatible, :conditions => ["salary_head_type = ?",'Allowance']
  scope :subtotal_compatible, :conditions => ["salary_head_type = ?",'SlipSubtotal']
  scope :deduction_compatible, :conditions => ["salary_head_type = ?",'Deduction']

  #scope :allowance_compatible, :conditions => ["salary_head_type = ?",'Allowance']
  #scope :subtotal_compatible, :conditions => ["salary_head_type = ?",'SlipSubtotal']
  #scope :deduction_compatible, :conditions => ["salary_head_type = ?",'Deduction']

  #TODO tried to solve the  defaults.
  #defaults :tax_formula => "TaxFormula"

  after_initialize :defaults

  def defaults
    self.tax_formula = "TaxFormula"
  end
