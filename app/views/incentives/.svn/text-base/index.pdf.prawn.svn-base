...
pdf.header [0,740] do
  ...
  pdf.text "Incentives", :align=>:center, :size=> sm+3
end
pdf.bounding_box [-10,680], :width=>570 ,:height => 650 do
  @incentives.each do |month,incentives|
    pdf.text "For month #{Date.new(@this_year,month,1).to_s(:month_and_year)}", :align => :center, :size => sm+1
    pdf.table incentives.map{|incentive| [incentive.employee.name, incentive.amount]},
