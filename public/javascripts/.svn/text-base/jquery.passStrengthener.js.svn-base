...
(function(jQuery){ 
	...
	jQuery.fn.passStrengthener = function(options)
        ...
        //common_words :[],
			...
			var allCSSArr = [];
     // var common_words = new Array("password","user","admin","login","email");
          ...
          var score = teststrength(jPass.val(), userID,common_words, opts);
	...
	function teststrength(password, username, common_words, option)
	{
	    //password < 4
    ...
    //check for username in password
      ...
      if(password.match(username) != null){
        return 'same';
      }
      ...
      jQuery.each(common_words, function(i){
        if(password == common_words[i])
          score -= 15;
      })
